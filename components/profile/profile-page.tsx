...
export function AccountPage({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {profile?.account_kind === "guest" ? <Card className="border-success/20 bg-success/5"><CardContent className="flex gap-3 p-4"><ShieldCheck className="mt-0.5 size-5 shrink-0 text-success" /><div><strong className="text-sm">บัญชี Guest นี้จำอยู่ในเบราว์เซอร์ปัจจุบัน</strong><p className="mt-1 text-xs leading-5 text-muted-foreground">เชื่อม Google ในหน้าตั้งค่าเพื่อเก็บข้อมูลเดิมและใช้งานต่อจากอุปกรณ์อื่นได้</p></div></CardContent></Card> : null}
