...
import { Check, Hash, LockKeyhole } from "lucide-react";
...
export function PredictionsPage() {
  ...
  useEffect(() => { let active = true; void createClient().from("lottery_draws").select("id,draw_date").eq("status", "scheduled").order("draw_date").then(({ data }) => { if (active) { const next = (data ?? []) as Draw[]; setDraws(next); setDrawId(next[0]?.id ?? ""); } }); return () => { active = false; }; }, []);
